 ...
 * </ol>
 */
public class AnterosMFRC522 implements Closeable {
	...
	public AnterosMFRC522(int chipSelect, int resetGpio) {
		this(SpiConstants.DEFAULT_SPI_CONTROLLER, chipSelect, resetGpio);
	}

	@SuppressWarnings("resource")
	public AnterosMFRC522(int controller, int chipSelect, int resetGpio) {
		this(controller, chipSelect, new DigitalOutputDevice(resetGpio, true, false));
	}

	public AnterosMFRC522(int controller, int chipSelect, DigitalOutputDevice resetPin) {
	...
	public void init() {
		...
		/*
		 * writeRegister(T_MODE_REG, (byte) 0x8D); // Tauto=1; f(Timer) =
		 * 6.78MHz/TPreScaler writeRegister(T_PRESCALER_REG, (byte) 0x3E); //
		 * TModeReg[3..0] + TPrescalerReg
		 * 
		 * // 30 writeRegister(T_RELOAD_REG_MSB, (byte) 0);
		 * writeRegister(T_RELOAD_REG_LSB, (byte) 0x1E);
		 * 
		 * writeRegister(TX_ASK_REG, (byte) 0x40); // 100%ASK writeRegister(MODE_REG,
		 * (byte) 0x3D); // CRC Initial value 0x6363 ???
		 */
	...
	private Response communicateWithPICC(PcdCommand command, byte waitIRq, byte[] sendData, byte validBits,
		...
		if (command != PcdCommand.MF_AUTHENT) {
			...
			/*
			 * back_data = new byte[back_len]; for (int index=0; index<back_len; index++) {
	...
	public UID select(byte validBits) {
		...
		// Byte 0: SEL Indicates the Cascade Level: PICC_CMD_SEL_CL1, PICC_CMD_SEL_CL2
		// or PICC_CMD_SEL_CL3
	 ...
	 */
	/*
	 * public StatusCode ntag216Auth(byte[] passWord, byte pACK[]) //Authenticate
	 * with 32bit password { // TODO: Fix cmdBuffer length and rxlength. They really
	 * should match. // (Better still, rxlength should not even be necessary.)
	 * 
	 * StatusCode result; byte cmdBuffer[18]; // We need room for 16 bytes data and
	 * 2 bytes CRC_A.
	 * 
	 ...
	 * 
	 * // Transceive the data, store the reply in cmdBuffer[] byte waitIRq = 0x30;
	 * // RxIRq and IdleIRq // byte cmdBufferSize = sizeof(cmdBuffer); byte
	 * validBits = 0; byte rxlength = 5; result =
	 * PCD_CommunicateWithPICC(PCD_Transceive, waitIRq, cmdBuffer, 7, cmdBuffer,
	 * &rxlength, &validBits);
	 * 
	 * pACK[0] = cmdBuffer[0]; pACK[1] = cmdBuffer[1];
	 * 
	 * if (result!=STATUS_OK) { return result; }
	 * 
	 * return STATUS_OK; } // End PCD_NTAG216_AUTH()
	 */
	...
	public StatusCode mifareTransceive(byte[] sendData, boolean acceptTimeout) {
		...
		/*
		 * C++ Code: byte cmdBuffer[18]; // We need room for 16 bytes data and 2 bytes
		 * CRC_A. // Copy sendData[] to cmdBuffer[] and add CRC_A memcpy(cmdBuffer,
		 * sendData, sendLen); result = PCD_CalculateCRC(cmdBuffer, sendLen,
		 * &cmdBuffer[sendLen]); if (result != STATUS_OK) { return result; } sendLen +=
		 * 2;
		 */
	 ...
	 */
	/*
	 * public void mifareSetAccessBits( byte *accessBitBuffer, ///< Pointer to byte
	 * 6, 7 and 8 in the sector trailer. Bytes [0..2] will be set. byte g0, ///<
	 * Access bits [C1 C2 C3] for block 0 (for sectors 0-31) or blocks 0-4 (for
	 * sectors 32-39) byte g1, ///< Access bits C1 C2 C3] for block 1 (for sectors
	 * 0-31) or blocks 5-9 (for sectors 32-39) byte g2, ///< Access bits C1 C2 C3]
	 * for block 2 (for sectors 0-31) or blocks 10-14 (for sectors 32-39) byte g3
	 * ///< Access bits C1 C2 C3] for the sector trailer, block 3 (for sectors 0-31)
	 * or block 15 (for sectors 32-39) ) { byte c1 = ((g3 & 4) << 1) | ((g2 & 4) <<
	 * 0) | ((g1 & 4) >> 1) | ((g0 & 4) >> 2); byte c2 = ((g3 & 2) << 2) | ((g2 & 2)
	 * << 1) | ((g1 & 2) << 0) | ((g0 & 2) >> 1); byte c3 = ((g3 & 1) << 3) | ((g2 &
	 * 1) << 2) | ((g1 & 1) << 1) | ((g0 & 1) << 0);
	 * 
	 * accessBitBuffer[0] = (~c2 & 0xF) << 4 | (~c1 & 0xF); accessBitBuffer[1] = c1
	 * << 4 | (~c3 & 0xF); accessBitBuffer[2] = c3 << 4 | c2; } // End
	...
	public void dumpMifareClassicSectorToConsole(UID uid, byte[] key, byte sector) {
		...
		// g[3] Access bits for the sector trailer, block 3 (for sectors 0-31) or block
		// 15 (for sectors 32-39)
		// g[2] Access bits for block 2 (for sectors 0-31) or blocks 10-14 (for sectors
		// 32-39)
