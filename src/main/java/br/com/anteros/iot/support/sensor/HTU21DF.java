...
import br.com.anteros.iot.support.utils.TimeUtil;
...
public class HTU21DF {
	...
	public boolean begin()
		...
		try {
			...
			r = htu21df.read();
			if (verbose) {
				System.out.println("DBG: Begin: 0x" + StringUtils.leftPad(Integer.toHexString(r), 2, "0"));
			}
		} catch (Exception ex) {
	...
	public float readTemperature()
		...
		if (verbose) {
			System.out.println("Read Temp: Written 0x" + StringUtils.leftPad(Integer.toHexString((HTU21DF_READTEMP & 0xff)), 2, "0"));
		}
		htu21df.write((byte) (HTU21DF_READTEMP)); //  & 0xff));
		TimeUtil.delay(50); // Wait 50ms
		byte[] buf = new byte[3];
	  /*int rc  = */
		...
		if (verbose) {
			System.out.println("Temp -> 0x" + StringUtils.leftPad(Integer.toHexString(msb), 2, "0") + " " + "0x" +
					StringUtils.leftPad(Integer.toHexString(lsb), 2, "0") + " " + "0x" + StringUtils.leftPad(Integer.toHexString(crc), 2, "0"));
    ...
    /* int rc  = */
		...
		if (verbose) {
			System.out.println("Hum -> 0x" + StringUtils.leftPad(Integer.toHexString(msb), 2, "0") + " " + "0x" +
					StringUtils.leftPad(Integer.toHexString(lsb), 2, "0") + " " + "0x" + StringUtils.leftPad(Integer.toHexString(crc), 2, "0"));
