...
import br.com.anteros.iot.support.utils.EndianReaders;
...
public class BME280 {
	...
	private String displayRegister(int reg) {
		return String.format("0x%s (%d)", StringUtils.leftPad(Integer.toHexString(reg & 0xFFFF).toUpperCase(), 4, "0"), reg);
	}
	...
	private int readRawTemp() throws Exception {
		...
		sleepTime = sleepTime + 0.0023 * (1 << mode) + 0.000575;
		TimeUtil.delay((long) (sleepTime * 1_000L));
