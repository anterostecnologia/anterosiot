...
import br.com.anteros.iot.support.lcd.oled.ImgInterface;
import br.com.anteros.core.utils.StringUtils;
import br.com.anteros.iot.support.lcd.oled.CharacterMatrixes;
	...
	public void image(ImgInterface img, int topLeftX, int topLeftY, Mode mode) {
		...
		for (int col = 0; col < w; col++) {
			for (int row = 0; row < (h / 8); row++) {
				String bitMapCol = StringUtils.leftPad(Integer.toBinaryString(imgBuf[col + (w * row)]), 8, "0").replace('0', (mode == Mode.WHITE_ON_BLACK ? ' ' : 'X')).replace('1', (mode == Mode.WHITE_ON_BLACK ? 'X' : ' '));
